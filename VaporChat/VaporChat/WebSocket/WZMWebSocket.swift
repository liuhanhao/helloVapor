...
final class WZMWebSocketEventSingle {
    ...
    func registeredMessage(handler: @escaping ((String) -> Void)) {
        WZMWebSocketService.sharedInstance.message_signal.observeResult { result in
            switch result {
    ...
    func registeredEvent(handler: @escaping ((Bool) -> Void)) {
        WZMWebSocketService.sharedInstance.event_signal.observeResult { result in
            switch result {
                case .success(let value):
                    handler(value)
                case .failure(let error):
...
final class WZMWebSocketService: WebSocketDelegate {
    /*
    ...
    private var socket = WebSocket.init(request: URLRequest.init(url: URL.init(string: WZMChatURL.ipAddress + "chat/")!))
    
    // 单例
    static let sharedInstance = WZMWebSocketService()
    ...
    func didReceive(event: WebSocketEvent, client: WebSocket) {
        ...
        switch event {
            ...
            case .ping(_):
                ...
                let data = WZMHexadecimal.bytesArrayConversionData(bytesArray: buffer)
