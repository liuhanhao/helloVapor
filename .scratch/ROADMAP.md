...
- 需先决策：未读以服务端已读位点为准（可跨端同步）还是纯前端本地计数（简单但换设备就丢）。建议记 ADR。
...
- **B1 未读计数已决策：先做前端本地计数。** 原本在「服务端已读位点（可跨端同步）」与「前端本地计数（换设备就丢）」之间摇摆。定后者，理由不是它更好，而是**群聊还没落地，未读的语义尚未定型**——群消息是否计入未读、@提及要不要单独计数，这些都会改变数据模型。现在把已读位点做进服务端，等于给一个还没定型的语义提前建模。等群聊上线、跨端不一致真的被抱怨了再补，那时需求是具体的。
